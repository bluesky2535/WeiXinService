		...
		try {
			...
			if (MessageUtil.MSGTYPE_TEXT.equals(msgType)) {
				if(content.startsWith("翻译")){
					try {
						respone = MessageUtil.initTranslate(toUser, fromUser,
								content);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					respone = MessageUtil.initText(toUser, fromUser,
							MessageUtil.keyWord(content));
					...
					respone = MessageUtil.initNoEffectMessage(toUser, fromUser);
				}
